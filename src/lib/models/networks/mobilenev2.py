...
class MobileNetSeg(nn.Module):
    def __init__(self, base_name, heads, head_conv=24, pretrained=True):
        ...
        # print('mobilenetv2_{}'.format(num_layers)) # **
...
#    @dict2list         # 转onnx的时候需要将输出由dict转成list模式
    def forward(self, x):
        ...
        ret = {}
        for head in self.heads:
            # print("head    ", self.__getattr__(head))
...
# num_layers  : [10 , 5]
def get_mobilenetv2(num_layers, heads, head_conv=24, device=None):
...
if __name__ == '__main__':
    ...
    model = get_mobilenetv2(10, {'hm':1, 'hm_offset':2, 'wh':2, 'landmarks':10}, head_conv=24)         # hm reference for the classes of objects//这个头文件只能做矩形框检测
    res = model(input)
    print(model)
