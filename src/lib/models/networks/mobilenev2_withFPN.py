...
model_urls = {
    ...
    # 'mobilenet_v2': '../exp/ctdet/imagenet/mobilenetv2_128x128-fd66a69d.pth',
}
...
class MobileNetUp(nn.Module):
    ...
    def forward(self, layers):
        ...
        fpn_6 = self.conv_last(x)
        
        up = getattr(self, 'up_0')
        x = up([x,layers[2]])
        fpn_4 = self.conv_last(x)
        
        up = getattr(self, 'up_1')
        x = up([x,layers[1]])
        fpn_2 = self.conv_last(x)
        
        up = getattr(self, 'up_2')
        x = up([x,layers[0]])
        fpn_1 = self.conv_last(x)
        
#         for i in range(0,len(layers)-1):
#             up = getattr(self, 'up_{}'.format(i))
#             x = up([x,layers[len(layers)-2-i]])
#         x = self.conv_last(x)
        return fpn_6, fpn_4, fpn_2, fpn_1

class MobileNet_withFPN(nn.Module):
    def __init__(self, base_name, heads, head_conv=24, pretrained = True):
        super(MobileNet_withFPN, self).__init__()
        self.heads = heads
...
#    @dict2list         # 转onnx的时候需要将输出由dict转成list模式
    def forward(self, x):
        x = self.base(x)
        x6, x4, x2, x1 = self.dla_up(x)
        x6_up8 = F.interpolate(x6, scale_factor=8, mode='bilinear', align_corners=True)
        x4_up4 = F.interpolate(x4, scale_factor=4, mode='bilinear', align_corners=True)
        x2_up2 = F.interpolate(x2, scale_factor=2, mode='bilinear', align_corners=True)
        # x = (x6_up8*0.25 + x4_up4*0.25 + x2_up2*0.25 + x1*0.25)
        # x = (x6_up8 + x4_up4 + x2_up2 + x1) / 4
        # x = (x6_up8*0.25 + x4_up4*0.25 + x2_up2*0.25 + x1*0.25) / 4
        x = (x6_up8*0.1 + x4_up4*0.2 + x2_up2*0.3 + x1*0.4) / 4
        
        ret = {}
        for head in self.heads:
            ret[head] = self.__getattr__(head)(x)
        return [ret]#, {'x6': x6, 'x4': x4, 'x2': x2, 'x1': x1}
...
# num_layers  : [10 , 5]
def get_mobilenetv2_withFPN(num_layers, heads, head_conv=24, device=None):
    model = MobileNet_withFPN('mobilenetv2_{}'.format(num_layers), heads,
                pretrained=True,
                head_conv=head_conv)
    return model
...
if __name__ == '__main__':
    ...
    model = get_mobilenetv2_withFPN(10, {'hm':1, 'hm_offset':2, 'wh':2, 'landmarks':10}, head_conv=24)         # hm reference for the classes of objects//这个头文件只能做矩形框检测
    res = model(input)
    print(model)
    print(res[0]['hm'].shape)
    print(res[0]['hm_offset'].shape)
    print(res[0]['wh'].shape)
    print(res[0]['landmarks'].shape)
