...
class MobileNet_withFPNbicubic(nn.Module):
    def __init__(self, base_name, heads, head_conv=24, pretrained = True):
        super(MobileNet_withFPNbicubic, self).__init__()
        self.heads = heads
...
#    @dict2list         # 转onnx的时候需要将输出由dict转成list模式
    def forward(self, x):
        ...
        x6_up8 = F.interpolate(x6, scale_factor=8, mode='bicubic', align_corners=True)
        x4_up4 = F.interpolate(x4, scale_factor=4, mode='bicubic', align_corners=True)
        x2_up2 = F.interpolate(x2, scale_factor=2, mode='bicubic', align_corners=True)
...
# num_layers  : [10 , 5]
def get_mobilenetv2_withFPNbicubic(num_layers, heads, head_conv=24, device=None):
    model = MobileNet_withFPNbicubic('mobilenetv2_{}'.format(num_layers), heads,
                pretrained=True,
                head_conv=head_conv)
    return model
...
if __name__ == '__main__':
    ...
    model = get_mobilenetv2_withFPNbicubic(10, {'hm':1, 'hm_offset':2, 'wh':2, 'landmarks':10},head_conv=24)         # hm reference for the classes of objects//这个头文件只能做矩形框检测
    res = model(input)
    print(model)
